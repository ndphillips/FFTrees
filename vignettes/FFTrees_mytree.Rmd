...
When looking at **Figure 3**, we first move down on the right (from retrieving sets of FFT definitions to manipulating individual FFTs) and then move up on the left (to re-convert individual FFTs into sets of FFT definitions). 
...
This is the job of `read_fft_df()`: 
...
```{r fft-read-fft}
(fft_1 <- read_fft_df(tree_dfs, tree = 1))
```
...
Use the set of tree trimming and editing functions to manipulate the definitions of individual FFTs:
...
```{r fft-reorder-nodes}
(my_fft_1 <- reorder_nodes(fft_1, order = c(1, 3, 2)))
```

- `flip_exits()`

As every non-final node allows for two alternative exits, a tree containing $n$\ nodes has `2^{(n-1)}`\ exit structures. 
The $3$\ alternative exit structures to `my_fft_1` can be obtained by flipping the exit directions of the first, second, or both non-final nodes:
...
For instance, the sequence of four steps leading to `my_fft_2` (above) can be combined into: 

```{r fft-pipes, message = FALSE}
library(magrittr)
...
#### Re-converting and collecting tree definitions

- `write_fft_df()` converts a single FFT from the tidy data frame (with 1-row-per-node) into the 1-row-per FFT format used by `add_fft_df()`.

#### Applying sets of tree definitions to data 

- Provide sets of FFT definitions to the `tree.definitions` argument of `FFTrees()` to evaluate FFTs on data. 
...
If we wanted to change more aspects of\ `x` (e.g., use different `data` or `goal` settings), we could create a new `FFTrees` object without supplying the previous object\ `x`, as long as the FFTs defined in `tree.definitions` correspond to the settings of `formula` and `data`. 
