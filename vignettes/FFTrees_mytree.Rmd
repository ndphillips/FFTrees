...
- **Cycle\ 1**: The main `FFTrees()` function creates `FFTrees` objects `x` with tree definitions. We can obtain them by `get_fft_df(x)` or use alternative FFT definitions by providing them as inputs to its `tree.definitions` argument.

- **Cycle\ 2**: Conversion between two tree definition formats (both data frames):
    1. `read_fft_df()`
    2. `write_fft_df()`

- **Cycle\ 3**: Manipulating individual tree definitions by 6\ tree trimming functions.
