...
The **FFTrees** package makes it very easy to visualize fast-and-frugal trees (FFTs) and evaluate their performance. 

The main function `FFTrees()` creates fast-and-frugal trees (FFTs) as R objects of the type `FFTrees`. 
We visualize an `FFTrees` object\ `x` by plotting it (i.e., evaluating `plot(x)`). 

Two key arguments for plotting are `what` and `tree`: 
Whereas the `tree` argument allows selecting between different trees in\ `x` (using `tree = 1` by default), 
the `what` argument distinguishes between four main types of plots: 
...
3. `plot(x, what = 'cues')` visualizes the current cue accuracies in ROC\ space (by calling the `showcues()` function).

4. `plot(x, what = 'roc')` visualizes a performance comparison of FFTs and competing algorithms in ROC\ space. 
...
- `what` should be visualized? The `what` argument selects contents to be plotted: 
    ...
    - To only visualize the performance comparison (for different FFTs and competing algorithms) in ROC\ space, we specify `what = "roc"`.
    
<!-- Example what = 'tree': -->
    
```{r titanic-what-tree, fig.align = 'center', fig.height = 5, fig.width = 8, out.width = "75%", fig.cap="**Figure 3**: A plain FFT diagram (without statistics)."}
# Show only the tree diagram:
plot(titanic.fft, what = "tree")
```

<!-- Example what = 'roc': -->

```{r titanic-what-roc, fig.align = 'center', fig.height = 6, fig.width = 7, out.width = "75%", fig.cap="**Figure 4**: Performance comparison of FFTs in ROC space."}
# Show only the performance comparison in ROC space:
plot(titanic.fft, what = "roc")
```

- When the main options for `what` do not suffice, the various `show.` arguments (i.e., `show.header`, `show.tree`, `show.confusion`, `show.levels`, `show.roc`, `show.icons`, and `show.iconguide`) allow to selectively turn on or turn off specific elements of the overall plot. For example: 

```{r titanic-args, fig.align = 'center', fig.height = 7, fig.width = 9, out.width = "75%", fig.cap="**Figure 5**: Plotting selected elements."}
...
```{r titanic-train, fig.width = 7, fig.height = 7, fig.align='center', out.width = "75%", fig.cap="**Figure 6**: Plotting the best FFT on _training_ data."}
...
```{r titanic-test, fig.width = 7, fig.height = 7, fig.align='center', out.width = "75%", fig.cap="**Figure 7**: Plotting the best FFT on _test_ data."}
...
```{r titanic-viz-2, fig.width = 7, fig.height = 7, fig.align='center', out.width = "75%", fig.cap="**Figure 8**: Plotting Tree\ #2."} 
