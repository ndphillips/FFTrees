...
- `criterion_name`: The name of the (predicted) criterion variable. 
- `cue_names`: The names of all cue variables (predictors). 
- `formula`: The formula used to create the `FFTrees` object. 
- `trees`: Information on all trees in the object, with list elements that specify the definitions and performance of each FFT.
- `data`: Information on the datasets used to train and test the FFTs. 
- `params`: The parameters used for constructing FFTs. 
- `competition`: Models and statistics of alternative classification algorithms.
- `cues`: Information on all cue variables (predictors), with list elements that specify their thresholds and performance when training FFTs. 

<!-- OLD list (seems outdated): -->
...
```{r print-fftrees-object-default}
# Performance of the best tree, given the current goal:
heart.fft
```

Here is a description of the statistics provided by default: 
...
**Table 1**: Description of FFTs' basic accuracy and speed/frugality statistics. 

<!-- Same for alternative FFTs: -->
...
- _Exits, directions, and thresholds_: 
    ...
    - The corresponding cue _directions_ are\ `=` for the first cue, `=` for the second cue, and\ `>` for the third cue. Note that cue directions indicate how the tree _would_ make positive decisions _if_ it had a positive exit for that cue. If the tree has a positive exit for the given cue, then cases that satisfy this threshold and direction are classified as having a _positive_ criterion value. However, if the tree has only a negative exit for a given cue, then cases that do _not_ satisfy the given thresholds are classified as _negative_. 
...
2. Otherwise, if `cp` is not equal to\ `a`, predict a _negative_ value. 
