...
```{r iris-plot-cues}
# Plot cue values for iris FFTrees: 
plot(iris.fft, 
     what = "cues")
```

It looks like the cues `pet.wid` and `pet.len` are the best predictors. 
Based on this, we should expect the final trees will likely use just one or both of these cues
...
```{r iris-plot-fft}
# Plot best FFT in iris FFTrees: 
plot(iris.fft)
```

Indeed, it looks like the best tree only uses the `pet.wid` and `pet.len` cues. 
...
Importantly, FFTs (and the **FFTrees** package) help us to render these trade-offs more transparent. 
