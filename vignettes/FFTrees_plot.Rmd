...
Let's plot one of the trees (the best one, given our current goal):
...
If a testing dataset separate from the one used to build the tree is used, we'll see a "Prediction" label. 
    ...
    - A range of vertical levels show the tree's cumulative performance in terms of frugality measures (`mcu` and `pci`) and various accuracy measures (sensitivity, specificity, accuracy, and balance accuracy (see [Accuracy statistics](FFTrees_accuracy_statistics.html) for details). 
...
```{r titanic-stats, fig.align = 'center', fig.height = 6.5, fig.width = 10, out.width = "75%", fig.cap="**Figure 3**: A plain FFT plot (without statistics)."}
