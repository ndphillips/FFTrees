...
- `blood` -- Which patients donate blood? [source](https://archive.ics.uci.edu/ml/datasets/Blood+Transfusion+Service+Center)
...
### Preparations for applying the describe_data functions to all data sets
## When new data sets are included, add their info so that they will also be shown in the vignette-table!


# list all data sets 
data_list <- list(blood, breastcancer, car, contraceptive, creditapproval, fertility, forestfires, heartdisease, iris.v, mushrooms, sonar, titanic, voting, wine) 

# vector with all names of the data sets
data_names <- c("blood", "breastcancer", "car", "contraceptive", "creditapproval", "fertility", "forestfires", "heartdisease", "iris.v", "mushrooms", "sonar", "titanic", "voting", "wine")

# vector with all criterion names
criterion_names <- c("donation.crit", "diagnosis", "acceptability", "cont.crit", "crit", "diagnosis","fire.crit", "diagnosis", "virginica", "poisonous", "mine.crit", "survived", "party.crit", "type") 

# vector with all criterion values that we are interested in 
baseline_values <- c(1, TRUE, "acc", TRUE, TRUE,"O", TRUE, TRUE, TRUE, TRUE, TRUE, TRUE, TRUE, "red")



### QUESTIONS:
# diagnosis in fertility are N and O, what does that mean?
# what is the criterion at forestfires???


# use combined lists/vectors and apply describe_data function
result_list <- mapply(describe_data, data = data_list, data_name = data_names, criterion_name = criterion_names, baseline_value = baseline_values, SIMPLIFY = FALSE)

# combine results in one dataframe
combined_result <- do.call(rbind, result_list)


# round baseline values for briefity
combined_result$Baseline_pct<- round(combined_result$Baseline_pct, 1)

# rename columns for better interpretability
colnames(combined_result) <- c("Name", 
                                 "Number of Cases",
                                 "Criterion", 
                                 "Baseline (TRUE) in Percent",
...
kable(combined_result, "html") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"))


# DT::datatable(combined_result, options = list(pageLength = 14),
#               colnames = c(
#     "Number of Cases" = "Cases_n",
#     "Baseline (TRUE) in Percent" = "Baseline_pct",
#     "Number of Predictors" = "Predictors_n",
#     "Number of NAs" = "NAs_n",
#     "NAs in Percent" = "NAs_pct" ))
