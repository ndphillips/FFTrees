...
The following example follows the tutorial presented in @phillips2017FFTrees (freely available in [html](https://journal.sjdm.org/17/17217/jdm17217.html) | [PDF](https://journal.sjdm.org/17/17217/jdm17217.pdf)):
...
For definitions of all accuracy statistics, see the [accuracy statistics](AccuracyStatistics.html) vignette.
...
```{r fft-cues}
# Plot cue accuracies (in ROC space):
plot(heart.fft, 
     data = "test", 
     what = "cues")
```
...
Here is the resulting tree: (It's actually not too bad, although the first node is pretty worthless)

```{r plot-my-fft, fig.width = 6.5, fig.height = 6}
plot(my.heart.fft)
```

See the [Defining an FFT verbally](FFTrees_mytree.html) vignette for details on defining FFTs from verbal descriptions. 
