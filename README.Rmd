...
Let's create a fast-and-frugal tree predicting heart disease status (_Healthy_ vs. _Diseased_) based on a `heart.train` dataset, and test it on `heart.test` a testing dataset.
...
Here is `heart.train` (the training / fitting dataset) which contains data from `r nrow(heart.train)` patients:
...
```{r example-heart, fig.width = 6, fig.height = 6, out.width = "80%"}
...
4. Otherwise, predict _Healthy_. 

The conditions can directly be supplied to the `my.tree` argument of `FFTrees()`: 

```{r example-heart-verbal, fig.width = 6, fig.height = 6, out.width = "80%"}
...
[ [html](https://journal.sjdm.org/17/17217/jdm17217.html) | [PDF](http://journal.sjdm.org/17/17217/jdm17217.pdf) ] 

We had a lot of fun creating **FFTrees** and hope you like it too! 
We have an article introducing the **FFTrees**  package in the journal _Judgment and Decision Making_, entitled [FFTrees: A toolbox to create, visualize,and evaluate fast-and-frugal decision trees](https://journal.sjdm.org/17/17217/jdm17217.pdf). 
