...
When printing an `FFTrees` object, we automatically get the performance on the `test` data (i.e., for _prediction_, rather than _fitting_) and obtain the information about the tree with the highest value of the `goal` statistic. 
...
To obtain the same information for another FFT of an `FFTrees` object, we can call `print()` with a numeric `tree` parameter. 
...
Alternatively, we could visualize the same tree and its performance by calling `plot(heart.fft, tree = 3, data = "test")`. 
