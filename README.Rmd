...
A `heart.train` set for fitting decision trees, and `heart.test` set for a testing the resulting trees. 
...
c_train <- paste0("**Table 1**: First lines of the `heart.train` subset (using data of ", n_train, " patients for fitting/training FFTs).")

knitr::kable(head(heart.train), caption = c_train)
```

- `heart.test` (the testing / prediction data) describes `r nrow(heart.test)` different patients:

```{r data-test, echo = FALSE}
n_test <- nrow(heart.test)
c_test <- paste0("**Table 2**: First lines of the `heart.test` subset (used to predict `diagnosis` for ", n_test, " new patients).")

knitr::kable(head(heart.test), caption = c_test)
```

For this task, the (to-be predicted) criterion variable is `diagnosis` ---\ a logical vector (or column) indicating the true state for each patient (`TRUE` or\ `FALSE`, i.e., whether or not the patient suffers from heart disease). 
Most of the variables in both datasets are potential predictors. 
...
The **FFTrees** package creates FFTs and provides tentative answers to all these questions. 
...
By\ 2023, over 100\ scientific publications that have used or cited **FFTrees** (see [Google Scholar](https://scholar.google.com/scholar?oi=bibs&hl=en&cites=205528310591558601) for the full list). 
Examples include: 
