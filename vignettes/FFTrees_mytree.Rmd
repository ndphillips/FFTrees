...
For example, let's look at the `heartdisease` data to find out how the predictor variables `sex`, `age` and\ `thal` predict the criterion variable (`diagnosis`):

```{r heart-data, echo = FALSE}
knitr::kable(head(heartdisease[c("sex", "age", "thal", "diagnosis")]))
```

Table: **Table 1**: Three cues and binary criterion variable for first cases of the `heartdisease` data.
...
Now, let's pass the verbal `my.tree` argument to `FFTrees()` to create a corresponding FFT and apply it to the `heartdisease` data:

```{r create-my-tree}
# Pass a verbal description of an FFT to FFTrees (using my.tree): 
...
# Inspect an FFTrees object:
plot(my_fft)  # plot overview
# my_fft      # print summary
```

As it turns out, this FFT is pretty bad: 
It has high sensitivity (of\ 91%), but its abysmal specificity (of only\ 10%) allows for many false alarms. 
...
# Inspect:
plot(my_fft_2)  # plot overview
# my_fft_2      # print summary 
```
...
# 2. Reverse the cues (i.e, final cue is numeric): ------ 
my_fft_2 <- FFTrees(diagnosis ~.,
                    data = heartdisease,
...
# When the description predicted False instances first, the cue direction is reversed. 
...
# 3. Example used by Nathaniel: 
my_fft_3 <- FFTrees(formula = diagnosis ~., 
                    data = heart.train,
