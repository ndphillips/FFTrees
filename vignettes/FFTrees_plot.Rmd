...
For each passenger, we know in which class s/he travelled, as well as binary categories specifying age, sex, and survival information. 
...
- `stats`: To visualise a bare tree (without performance statistics), we can use `stats = FALSE`:
...
```{r titanic-check-plot-new-data, echo = FALSE, eval = FALSE}
# Check: Apply existing FFTs to new test data (in plot):

# +++ here now +++ 

# (1) Using titanic.fft: ------ 
test_data <- titanic[sample(1:nrow(titanic), 100), ]
dim(test_data)

predict(titanic.fft, newdata = test_data, type = "both")

plot(titanic.fft, data = test_data)  # provides a df

# (2) Using heart.fft: ------

# dim(heartdisease)
set.seed(2)
ix_train <- sample(1:nrow(heartdisease), 100)

heart_train <- heartdisease[ix_train, ]
heart_test <- heartdisease[-ix_train, ]
dim(heart_test)

# Train:
heart_fft <- FFTrees(diagnosis ~., data = heart_train)
plot(heart_fft)

# Predict:
plot(heart_fft, data = heart_test)
```
