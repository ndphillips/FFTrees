...
plot.FFForest <- function(x,
  ...
  if (is.null(palette)) {
    palette <- yarrr::piratepal("basel", length.out = length(frequencies), trans = .4)
  } else {
  ...
  # Barplot
  {
    ...
    par(xpd = TRUE)
    text(frequencies / trees.n, bp.vals[, 1],
      pos = 4,
  ...
  # Network plot
  {
    ...
    if (lo == "kk") {
      locations <- igraph::layout_with_kk(g)
    }
    if (lo == "dh") {
      locations <- igraph::layout_with_dh(g)
    }
    if (lo == "fr") {
      locations <- igraph::layout_with_fr(g)
    }
    ...
    if (min(locations[, 1]) < 0) {
      x.min <- min(locations[, 1]) * 1.1
    }
    if (max(locations[, 1]) < 0) {
      x.max <- max(locations[, 1]) * .9
    }
    if (min(locations[, 2]) < 0) {
      y.min <- min(locations[, 2]) * 1.1
    }
    if (max(locations[, 2]) < 0) {
      y.max <- max(locations[, 2]) * .9
    }

    if (min(locations[, 1]) > 0) {
      x.min <- min(locations[, 1]) * .9
    }
    if (max(locations[, 1]) > 0) {
      x.max <- max(locations[, 1]) * 1.1
    }
    if (min(locations[, 2]) > 0) {
      y.min <- min(locations[, 2]) * .9
    }
    if (max(locations[, 2]) > 0) {
      y.max <- max(locations[, 2]) * 1.1
    }
    ...
    for (i in 1:nrow(edges)) {
      is <- c(
        which(cue.names == edges[i, 1]),
        which(cue.names == edges[i, 2])
      )

      lines(
        x = c(locations[is[1], 1], locations[is[2], 1]),
