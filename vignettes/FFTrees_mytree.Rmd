...
The $3$\ exit structures in addition to `my_fft_1` can be obtained by flipping the exit directions of the first, second, or both non-final nodes. Each alternative can be achieved by applying the `flip_exits()` function to `my_fft_1`:
...
This creates an R object `my_tree_dfs` as a (non-tidy) data frame in which the definition of `my_fft_1` occupies a single line. 


Converting and adding more tree definitions to an existing set of FFT-definitions:
