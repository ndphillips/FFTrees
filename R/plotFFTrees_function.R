  ...
  if (what != "cues") {
    ...
    # --------------------------
    {
      ...
      # Check for problems and depreciated arguments
      {
        ...
        if (class(data) == "character") {
          if (data == "test" & is.null(x$trees$results$test$stats)) {
      ...
      if (comp == TRUE) {
        if (is.null(x$comp$lr$results) == FALSE) {
          lr.stats <- data.frame(
            "sens" = x$comp$lr$results[[data]]$sens,
            "spec" = x$comp$lr$results[[data]]$spec
          )
        }

        if (is.null(x$comp$cart$results) == FALSE) {
          lcart.stats <- data.frame(
            "sens" = x$comp$cart$results[[data]]$sens,
        ...
        if (is.null(x$comp$rf$results) == FALSE) {
          rf.stats <- data.frame(
            "sens" = x$comp$rf$results[[data]]$sens,
            "spec" = x$comp$rf$results[[data]]$spec
          )
        }

        if (is.null(x$comp$svm$results) == FALSE) {
          svm.stats <- data.frame(
            "sens" = x$comp$svm$results[[data]]$sens,
    ...
    if (show.bottom == TRUE) {{ # OBTAIN FINAL STATISTICS
      ...
      if (show.roc) {
        ...
        ## FFT
        {
          ...
          # Labels
          if (comp == TRUE & any(
            is.null(x$comp$lr$results) == FALSE,
