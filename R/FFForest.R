...
FFForest <- function(formula = NULL,
  ...
  result.ls <- parallel::mclapply(1:nrow(simulations), FUN = function(x) {
    if (!quiet) {
      cat(paste0(x, " of ", nrow(simulations), ", "))
    }
  ...
  if (is.null(data.test) == FALSE) {
    FFForest.Test.Decisions <- sapply(1:nrow(simulations), FUN = function(x) {
      pred <- apply.tree(
        data = data.test,
        formula = formula,
        tree.definitions = simulations[x, ]
      )$decision
    })

    FFForest.Test.Decisions <- rowMeans(FFForest.Test.Decisions) >= .5

    test.stats <- classtable(
      prediction.v = FFForest.Test.Decisions,
  ...
  # Get single surrogate tree with highest agreement with FFForest
  {
    ...
    if (length(surrogate.tree) > 1) {
      surrogate.tree <- sample(surrogate.tree, size = 1)
    }
  ...
  competitors <- as.data.frame(t(sapply(1:length(result.ls), FUN = function(x) {
    result.ls[[x]]$competitors
  })))
