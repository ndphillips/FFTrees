...
summary.df <- data.frame("train" = c(
                                      ...
                                      round(mean(x$tree.sim$mcpc.train), 2),
                                      round(mean(x$tree.sim$frugality.train), 2)
                                     ),
                         "test" = c(
                           ...
                           round(mean(x$tree.sim$mcpc.test), 2),
                           round(mean(x$tree.sim$frugality.test), 2)
                         )
                         )

row.names(summary.df) <- c("acc", "bacc", "sens", "spec", "mcpc", "frug")
