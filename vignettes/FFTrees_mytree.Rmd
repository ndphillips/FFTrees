...
Having edited and collected a set of tree definitions (in a data frame `my_tree_dfs`), we now can evaluate the corresponding FFTs on data. 
We can do this by providing our set of tree definitions (`my_tree_dfs`) to the `tree.definitions` of the main `FFTrees()` function. 
...
# Create a new FFTrees object y:
y <- FFTrees(object = x, 
             #
             tree.definitions = my_tree_dfs,    # provide set of FFT definitions !!!
             #
             main = "Heart Disease 2"           # new label
             )
```

Evaluate FFT performance:
...
Differences are small --- accuracy even identical. 
Not surprising, as changing the order of the 2nd and the 3rd cue has no effects on most cases. 


Note that we do not use training vs. test data, as no model is being fitted. 

```{r use-tree-definitions-02, eval = FALSE}
# Create a new FFTrees object z:
z <- FFTrees(formula = diagnosis ~ .,           # criterion and (all) predictors
...
Comparing our new with the set of old (or automatically created) FFTs:

```{r eval-FFTs-03, eval = FALSE}
(all_fft_dfs <- add_fft_df(my_tree_dfs, tree_dfs))

# Create a new FFTrees object a:
a <- FFTrees(formula = diagnosis ~ .,           # criterion and (all) predictors
             data = heartdisease,               # using full dataset
             #
             tree.definitions = all_fft_dfs,    # provide set of FFT definitions !!!
             #
             main = "Heart Disease 4",          # new label
             decision.labels = c("low", "high") # exit labels
             )
