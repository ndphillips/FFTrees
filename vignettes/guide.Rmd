---
title: "FFTrees: Fast-and-frugal decision trees"
author: "Nathaniel D. Phillips, Nathaniel.D.Phillips.is@gmail.com, http://ndphillips.github.io"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
bibliography: fft.bib
csl: apa.csl
vignette: >
  %\VignetteIndexEntry{FFTrees package overview}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

```{r, echo = F, message = F, results = 'hide'}
library(FFTrees)
library(knitr)
```

## Fast and Frugal Trees (FFTrees)

```{r, fig.width = 7, fig.height = 6.5, echo = TRUE, fig.align='center', echo = TRUE, message = FALSE}
# Create a fast-and-frugal tree (FFT) predicting heart disease
heart.fft <- FFTrees(formula = diagnosis ~.,
                     data = heart.train,
                     data.test = heart.test,
                     decision.labels = c("Low-Risk", "High-Risk"))

# Visualize the best training tree applied to the test data
plot(heart.fft,
     data = "test",
     main = "Heart Disease")
```


A fast-and-frugal tree (FFT) is a set of hierarchical rules for making decisions based on very little information (usually 4 or fewer). For example, the tree above uses data from to decide whether a patient has heart disease or not based on up to 3 pieces of information.

FFTs are simple, transparent decision strategies that use minimal information to make decisions [see @gigerenzer1999fast;@gigerenzer1999good]. They are frequently preferable to more complex decision strategies (such as Logistic Regression) because they rarely over-fit data [@gigerenzer2009homo] and are easy to interpret and implement in real-world decision tasks [@marewski2012heuristic]. They have been used in real world tasks from detecting depression [@jenny2013simple], to making fast decisions in emergency rooms [@green1997alters].

The purpose of the `FFTrees` package is to produce, compare, and display FFTs. The main function in the package is `FFTrees()` which takes formula `formula` and dataset `data` arguments and returns several FFTs which attempt to classify training cases into criterion classes. 

## How to use the package

To get started with `FFTrees`, we recomend looking at the [Heart Disease Tutorial](FFTrees_heart.html). This tutorial will take you through all of the basics steps of creating, visualising, and evaluating fast-and-frugal trees.

### Vignettes

Here is a complete list of the vignettes

| | Vignette Link| Description |
|--:|:------------------------------|:-------------------------------------------------|
|     1|    [Heart Disease Tutorial](FFTrees_heart.html)| A complete example of using FFTrees to model heart disease diagnosis     |
|      |    [Accuracy statistics](AccuracyStatistics.html)| Definitions of accuracy statistics used throughout the package|
|     2|    [Creating FFTrees with FFTrees()](FFTrees_function.html) |Description of  the main function `FFTrees()`|
|     |    [FFT construction](FFTrees_algorithm.html)|Description of the algorithm(s) used to grow FFTs     |
|      |    [Specifying trees directly with my.tree](FFTrees_mytree.html)|How to create FFTs directly without using one of the built-in algorithms|
|     3|    [Visualizing FFTrees with plot()](FFTrees_plot.html)|Plotting `FFTrees()` objects|
|     4|    [Examples of FFTrees](FFTrees_examples.html)|Lots of examples of FFTs from different datasets contained in the package|



## Please read our article and cite FFTrees!

We have a forthcoming article introducing the package in the Journal of [Judgment and Decision Making](http://www.sjdm.org)


If you use `FFTrees` in your work, please cite us so those of us without permanent positions can keep our jobs and continue developing the package:

```{r}
# Cite the package
citation("FFTrees")
```

To cite our article, use the following BibTeX entry:

```{r, eval = FALSE}
# Cite the article
@article{phillips2017FFTrees,
  title={FFTrees: A toolbox to create, visualise, and evaluate fast-and-frugal decision trees},
  author={Phillips, Nathaniel D. and Neth, Hansjoerg and Woike, Jan and Wolfgang, Gaissmaier},
  journal={Judgment and Decision Making},
  year={2017}
}
```

Phillips, N. D., Neth, H., Gaissmeier, W. & Woike, J. (2017). FFTrees: An R package to create, visualize, and implement fast-and-frugal decision trees. *Judgment and Decision Making*.


## Datasets

The package contains several datasets taken from the [UCI Machine Learning Repository](http://archive.ics.uci.edu/ml/) that you can use to play around with `FFTrees`

- `heartdisease` -- Patients suspected of having heart disease [source](http://archive.ics.uci.edu/ml/datasets/Heart+Disease)
- `breastcancer` -- Patients suspected of having breast cancer [source](https://archive.ics.uci.edu/ml/datasets/Breast+Cancer+Wisconsin+(Diagnostic))
- `titanic` -- Passengers on the Titanic
- `forestfires` -- Forest fire statistics [source](http://archive.ics.uci.edu/ml/datasets/Forest+Fires)
- `wine` -- Ratings of wine quality [source](http://archive.ics.uci.edu/ml/datasets/Wine)
- `income` -- Census data from > 30,000 US residents [source](http://archive.ics.uci.edu/ml/datasets/Adult)
- `bank` -- Bank marketing dataset [source](http://archive.ics.uci.edu/ml/datasets/Bank+Marketing)
- `voting` -- 1984 US congressional voting records [source](https://archive.ics.uci.edu/ml/datasets/Congressional+Voting+Records)


## Feature requests and bug reports

This package is constantly being updated. The latest developer version is always at [https://github.com/ndphillips/FFTrees](https://github.com/ndphillips/FFTrees). For comments, tips, additional references, and bug reports, please add an issue at [https://github.com/ndphillips/FFTrees/issues](https://github.com/ndphillips/FFTrees/issues) or email me at Nathaniel.D.Phillips.is@gmail.com

```{r fig.align = "center", out.width="50%", echo = FALSE}
knitr::include_graphics("../inst/FFTrees_Logo.jpg")
```


## Bibliography
