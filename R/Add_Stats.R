...
Add_Stats <- function(data,
    ...
    data$costout <- with(data, -1 * (hi * cost.outcomes$hi + fa * cost.outcomes$fa + mi * cost.outcomes$mi + cr * cost.outcomes$cr))
    data$cost <- data$costout - data$n * cost.each

    # reorder
    data <- data[, c("sens", "spec", "acc", "bacc", "wacc", "costout", "cost")]
