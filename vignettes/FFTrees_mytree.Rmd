...
The following example illustrates the interplay of these functions a typical workflow. 
...
The `tree_dfs` is a data frame with `r ncol(tree_dfs)` variables. 
...
As the "ifan" algorithm responsible for creating these trees yields a family of highly similar FFTs (as the FFTs vary only by their exits, and some truncate the last cue), we may want to explore alternative versions for these trees. 
