...
To verify that **FFTrees** interpreted our `my.tree` description as intended, let's check whether the FFT of `inwords(my_fft_2)` yields a description that corresponds to our intended\ FFT:
...
3. Manipulate the FFT (e.g., by changing nodes) 
4. Re-convert the new definition into the original data format 
5. Collect sets of changed FFT definitions and/or evaluate them on data  


Conceptually, this workflow boils down to first creating an FFT model for some data, then manipulating the model and re-evaluating it on the data. 
The tools involved in this workflow allow editing the grammar of FFTs. 


#### Overview: Manipulating FFT definitions 
...
The main `FFTrees()` function creates an `FFTrees` object\ `x` with corresponding tree definitions. 
The following two functions get and collect FFT definitions:  
...
- **Cycle\ 2**: Converting between two tree definition formats (both data frames):  
    ...
    2. `write_fft_df()` converts a single FFT from the tidy data frame (with 1-row-per-node) into the 1-row-per FFT format used by `FFTrees()` and `tree.definitions()`. 

- **Cycle\ 3**: Manipulating an individual tree definition by 6\ tree editing and trimming functions:  
    1. `add_nodes()` allows adding new nodes to an FFT; 
    2. `drop_nodes()` allows deleting nodes of an FFT;  
    3. `edit_nodes()` allows changing existing nodes of an FFT; 
    4. `flip_exits()` allows changing the exit direction of existing nodes; 
    5. `reorder_nodes()` allows changing the order of FFT nodes; 
    6. `select_nodes()` allows filtering existing nodes of an FFT.  


<!-- +++ here now +++  -->

ToDo: Illustrate a typical workflow in an example.
...
First, we use our default algorithms to create an `FFTrees` object\ `x`:
...
Rather than evaluating them individually (e.g., by using `print(x)` or `plot(x)` commands to inspect specific trees), we now get the set of tree definitions to select and modify individual FFTs.
This could be done by evaluating `x$trees$definitions` or `summary(x)$definitions`. 
Alternatively, we can use the `get_fft_df()` utility function to extract the set of generated tree definitions:
