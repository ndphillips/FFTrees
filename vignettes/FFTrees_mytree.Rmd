...
- Factor thresholds must be specified within brackets like `sex = [male]`. For factors with sets of values, values within a threshold should be separated by commas like `eyecolor = [blue,brown]`
...
Here's one more example using the `income` data. We'll create an FFT predicting whether a person has an income greater than 50K using the cues age, education, and hours.per.week:

```{r}
head(income[c("age", "education", "hours.per.week", "income.gt50")])
```

```{r}
my.income.tree <- "If age < 35, predict False.
                   If education = [Doctorate,Masters], predict True.
                   If hours.per.week < 40, predict False.
                   Otherwise, predict True"

income.fft <- FFTrees(income.gt50 ~., 
                      data = income, 
                      my.tree = my.income.tree,
                      comp = FALSE)

plot(income.fft)
```

Wow, that tree also has pretty terrible accuracy. That's what I get for trying to come up with my own intuitive tree..

### `tree.definitions`

```{r, echo = FALSE}
heart.fft <- FFTrees::FFTrees(diagnosis ~., data = heartdisease)
```
