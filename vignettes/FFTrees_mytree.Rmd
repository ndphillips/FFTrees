...
The R object\ `tree_dfs` is a data frame with `r ncol(tree_dfs)` variables. 
...
To do so, we first select an initial FFT that we can then manipulate further: 
...
The resulting R object\ `fft_1` selected the definition of tree\ 1
...
For instance, the above sequence of steps could be combined into: 
