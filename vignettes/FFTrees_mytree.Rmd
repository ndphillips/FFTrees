...
As we can see, this FFT is pretty bad --- it has a high sensitivity (of\ 91%), but suffers from an abysmal specificity (of only\ 10%). 
...
This alternative FFT (`my_fft_2`) performs much better overall ---\ and is nicely balancing sensitivity and specificity\ --- but is still far from perfect. 
...
<!-- Notes: -->
<!-- In `fftrees_wordstofftrees()`, "Otherwise" part is being ignored. --> 
<!-- In `fftrees_ffttowords()`, final sentence always predicts positive instances first. --> 

```{r testing-my-tree, echo = FALSE, eval = FALSE}
# 1. FFT with 2 cues (final cue is categorical): ------ 
my_fft_1 <- FFTrees(diagnosis ~.,
                    data = heartdisease,
                    my.tree = "If age < 45, predict False. 
                               If thal = {fd, normal}, predict False.     
                               Otherwise, predict the opposite.")

# inspect:
my_fft_1$trees$definitions
# Note: Corrected the error in final direction: 
...
# 2. Reverse cues (i.e, final cue is numeric): ------ 
my_fft_2 <- FFTrees(diagnosis ~.,
                    data = heartdisease,
                    my.tree = "If thal = {fd, normal}, predict False.     
                               If age > 45, predict True. 
                               Otherwise, predict the opposite.")
