  ...
  for (cue_i in 1:cue_n) {
    ...
    if (all(is.na(cue_i_v)) == FALSE) {
      ...
      # Step 2: Determine best direction and threshold for cue [cue_i_best]
      {
        ...
        # Numeric, integer
        if (substr(cue_i_class, 1, 1) %in% c("n", "i")) {
          cue_i_stats <- FFTrees:::fftrees_threshold_numeric_grid(
        ...
        if (substr(cue_i_class, 1, 1) %in% c("f", "c", "l")) {
          cue_i_stats <- FFTrees:::fftrees_threshold_factor_grid(
            thresholds = cue_i_levels,
            cue_v = cue_i_v,
            criterion_v = criterion_v,
            directions = directions,
      ...
      cue_i_best <- FFTrees:::fftrees_threshold_factor_grid(
        thresholds = NULL,
        cue_v = NULL,
        criterion_v = NULL,
        sens.w = sens.w,
