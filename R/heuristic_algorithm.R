...
heuristic.algorithm <- function(formula,
  ...
  for (i in 1:ncol(data.mm)) {
    if (class(data.mm[, i]) == "matrix") {
      data.mm[, i] <- as.logical(data.mm[, i])
    }
  }
  ...
  factors.log <- sapply(1:ncol(data.mf), FUN = function(x) {
    class(data.mf[, x]) %in% c("character", "factor")
  })
  ...
  if (algorithm == "zigzag") {
    ...
    for (node.i in 1:nrow(cue.acc)) {
      if (node.i == 1) {
        ...
        if (length(current.cue) > 1) {
          current.cue <- sample(current.cue, size = 1)
        }
      ...
      if (node.i > 1) {
        ...
        if (last.exit == 0) {
          ...
          if (length(current.cue) > 1) {
            current.cue <- sample(current.cue, 1)
          }
        }
