...
The main `FFTrees()` function creates an `FFTrees` object\ `x` with its tree definitions. 
    1. `get_fft_df()` extracts the set of current FFT definitions.
    2. `add_fft_df()` adds one or more FFT definitions to an existing set (i.e., collecting sets of FFT definitions). 
...
- **Cycle\ 3**: Manipulating an individual tree definition by 6\ tree trimming functions.
