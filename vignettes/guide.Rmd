...
```{r fig1, fig.width = 7, fig.height = 6.5, echo = TRUE, fig.align='center', echo = TRUE, message = FALSE, fig.cap="A fast-and-frugal tree (FFT) to predict heart disease risk."}
...
We have a forthcoming article introducing the package in the Journal of [Judgment and Decision Making](http://www.sjdm.org) called *FFTrees: A toolbox to create, visualise, and evaluate fast-and-frugal decision trees. We encourage you to read the full article to learn more about the history of FFTs and how they work.

If you use `FFTrees`, please cite us (so those of us with temporary academic positions can keep our jobs). You can either cite the package, or the article:
